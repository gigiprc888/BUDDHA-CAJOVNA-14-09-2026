...
export function BrandLogo({ compact }: Props) {
  return (
    <Link
      to="/"
      className="logo-lockup flex min-w-0 items-center gap-2 rounded-full bg-bg py-0.5 pr-2 text-ivory md:gap-3 md:pr-3"
      aria-label="Buddha čajovna · Teahouse & Shisha Lounge"
    >
      <span
        className={
          compact
            ? "logo-mark relative inline-grid size-10 shrink-0 place-items-center bg-bg"
      ...
      <span className="min-w-0 leading-[1.02]">
        <span className="block truncate font-serif text-[1.35rem] font-semibold tracking-[0.06em] text-ivory uppercase md:text-[1.95rem]">
          Buddha
        </span>
        <span className="block truncate font-serif text-[0.72rem] font-semibold tracking-[0.08em] text-gold uppercase md:text-[0.92rem]">
          Čajovna · Praha
        </span>
        <span
          className={
