...
function PinCard({ pin, i, featured, en }: { pin: Pin; i: number; featured?: boolean; en: boolean }) {
  ...
  const kombini = pin.href === "/kombini";
  ...
  return (
    <article
      ref={ref}
      className={
        (kombini ? "kombini-note " : "paper-note ") +
        "pin-enter relative px-5 py-6 " +
    ...
    >
      <span
        className={
          kombini
            ? "absolute -top-2 left-1/2 size-4 -translate-x-1/2 rounded-full bg-gold ring-2 ring-gold/40 shadow-[0_0_14px_rgb(232_184_78_/_0.9)]"
            : "absolute -top-2 left-1/2 size-4 -translate-x-1/2 rounded-full bg-gold shadow-[0_2px_10px_rgba(212,180,131,0.9)] ring-2 ring-bg/40"
        }
      />
      {tag && (
        <p className={"text-[11px] font-medium uppercase tracking-[0.22em] " + (kombini ? "text-gold" : "text-gold")}>
          {kombini ? "コンビニ · " : ""}
          {tag}
        </p>
      )}
      ...
      {kombini && (
        <a href="/kombini" className="mt-4 inline-block text-[11px] tracking-[0.18em] text-gold uppercase hover:text-ivory">
          {en ? "What a kombini is →" : "Co je kombini →"}
        </a>
      )}
      <p className={"mt-5 text-[11px] uppercase tracking-[0.18em] " + (kombini ? "text-gold/50" : "opacity-45")}>
        {pin.author} · {when}
      </p>
    </article>
  );
