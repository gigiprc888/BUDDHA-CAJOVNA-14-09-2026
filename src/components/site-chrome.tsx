...
import { useI18n, nextLocale, localeShort, localeName, LOCALES } from "@/lib/i18n";
...
import { WaysaMark } from "@/components/waysa-mark";
import { Pic } from "@/components/pic";
import { ToTop } from "@/components/to-top";

function navCurrent(href: string, pathname: string, hash: string) {
...
function CartIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="1.7" aria-hidden>
      <path d="M4 6h2.2l1.1 9.2a1.6 1.6 0 0 0 1.6 1.4h8.4a1.6 1.6 0 0 0 1.6-1.3L20 8H7" strokeLinecap="round" strokeLinejoin="round" />
      <circle cx="10" cy="19.2" r="1.15" fill="currentColor" stroke="none" />
      <circle cx="17" cy="19.2" r="1.15" fill="currentColor" stroke="none" />
    </svg>
  );
}
...
function CartMark() {
  ...
  const current = navCurrent("/nakup", pathname, "");
  const count = live && n > 0 ? n : 0;
  return (
    <>
      <Link
        to="/nakup"
        ...
        className={
          "cart-beat relative grid size-11 place-items-center text-gold lg:hidden" +
          (current ? " is-current" : "")
        }
      >
        <CartIcon />
        {count > 0 && (
          <span className="absolute right-1 top-1 grid size-3.5 place-items-center rounded-full bg-gold text-[8px] font-semibold text-bg">
            {count}
          </span>
        )}
      </Link>
      <Link
        to="/nakup"
        className={"nav-link hidden shrink-0 px-1 py-2 sm:text-[16px] lg:inline" + (current ? " is-current" : "")}
      >
        {t("nav.shop")}
        {count > 0 ? ` · ${count}` : ""}
      </Link>
...
export function SiteHeader() {
  ...
  const bookOn = pathname === "/rezervace";
  return (
    <>
      <header className="fixed inset-x-0 top-0 z-[200] border-b border-gold/20 bg-bg">
        <div>
        <div className="relative z-[1] flex flex-nowrap items-center gap-2 overflow-hidden px-3 py-2 md:gap-4 md:px-8 md:py-3">
          ...
          <nav className="hidden items-center gap-5 xl:gap-7 lg:flex">
            {links.map((l) => (
              <a
                key={l.href}
                href={l.href}
                aria-current={navCurrent(l.href, pathname, hash) ? "page" : undefined}
                className={
                  (l.pin ? "nav-link nav-pin" : "nav-link") +
                  (navCurrent(l.href, pathname, hash) ? " is-current" : "")
                }
              >
                {l.label}
              </a>
          ...
          <div className="relative z-30 flex shrink-0 items-center gap-1 sm:gap-3">
            <button
              type="button"
              ...
              aria-label="Language"
            >
              {localeShort(nextLocale(locale))}
            </button>
            <CartMark />
            <Link
              to="/rezervace"
              className={
                "shrink-0 rounded-full bg-gold px-3 py-1.5 font-serif text-[14px] font-medium tracking-[0.04em] text-bg hover:bg-ivory sm:px-5 sm:py-2 sm:text-[16px]" +
                (bookOn ? " underline decoration-bg/70 underline-offset-[6px]" : "")
              }
            >
              {t("nav.book")}
            </Link>
            <button
              type="button"
              className="relative z-30 grid size-11 place-items-center rounded-full border border-gold/50 text-gold lg:hidden"
      ...
      {open && (
        <div className="fixed inset-0 z-[400] lg:hidden">
          ...
          />
          <nav className="relative z-10 mt-[4.75rem] grid gap-1 border-t border-gold/15 bg-bg px-6 py-5">
            {links.map((l) => (
              <a
                key={l.href}
                href={l.href}
                onClick={() => setOpen(false)}
                className={
                  "py-2.5 font-serif text-3xl font-medium tracking-[0.04em] " +
                  (navCurrent(l.href, pathname, hash) ? "text-gold underline underline-offset-8" : l.href === "/#nastenka" ? "text-gold" : "text-ivory")
                }
              >
                {l.label}
              </a>
            ...
            {LOCALES.map((l) => (
              <button
                ...
                className={
                  "py-2.5 text-left font-serif text-2xl " + (locale === l ? "text-gold" : "text-ivory")
                }
              >
                {localeName(l)}
              </button>
...
export function SiteFooter() {
  ...
  return (
    <footer className="relative isolate overflow-hidden border-t border-gold/20 pb-24 md:pb-10">
      ...
      <div className="gold-flow relative z-[1]" aria-hidden>
        <span className="gold-flow-shine" />
      </div>
      ...
      <div className="relative z-[1] flex flex-col items-center gap-3 bg-paper px-5 py-8 text-paper-ink">
        ...
        <div className="flex flex-wrap items-center justify-center gap-8">
          ...
          <WaysaMark className="text-paper-ink hover:text-paper-mute" />
        </div>
