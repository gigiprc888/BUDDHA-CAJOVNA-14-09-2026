...
export const SEED: Pin[] = [
  ...
  },
  {
    id: "n-kombini",
    lane: "news",
    title: "Hledáme prostor pro kombini",
    titleEn: "Looking for a kombini space",
    body: "Japonské konbini v centru Prahy — ne večerka. Výloha na ulici, zhruba 40–120 m², provoz 24/7. Máte prostor? Napište.",
    bodyEn: "A Japanese konbini in central Prague — not a corner shop. Street window, about 40–120 m², open 24/7. Have a space? Write to us.",
    author: "Buddha",
    when: "teď",
    whenEn: "now",
    tag: "hledáme",
  ...
  },
  {
    ...
    title: "Hledáme prostor pro kombini",
    titleEn: "Looking for a kombini space",
    body: "Japonské konbini v centru Prahy — ne večerka. Výloha na ulici, 40–120 m², provoz 24/7. Napište na buddha.provozni@gmail.com.",
    bodyEn: "A Japanese konbini in central Prague — not a corner shop. Street window, 40–120 m², open 24/7. Write to buddha.provozni@gmail.com.",
    author: "Buddha",
    when: "teď",
    whenEn: "now",
    tag: "hledáme",
...
const KEY = "buddha-board-v2";
const KEY_LEGACY = "buddha-board-v1";

type Store = { extra: Pin[]; hide: string[]; edit: Record<string, Pin> };

function emptyStore(): Store {
  return { extra: [], hide: [], edit: {} };
}
  ...
  try {
    ...
    if (raw) {
      ...
      return {
        ...
        edit: parsed.edit && typeof parsed.edit === "object" ? parsed.edit : {},
      };
    }
    const legacy = JSON.parse(localStorage.getItem(KEY_LEGACY) || "[]") as Pin[];
    return {
      extra: legacy.filter((p) => !SEED.some((s) => s.id === p.id)),
      hide: [],
      edit: {},
    };
  } catch {
...
export function loadPins(): Pin[] {
  const s = readStore();
  const seed = SEED.filter((p) => !s.hide.includes(p.id)).map((p) => s.edit[p.id] ?? p);
  return [...s.extra, ...seed];
}

export function addPin(pin: Omit<Pin, "id" | "when"> & { when?: string }): Pin {
  const s = readStore();
  const row: Pin = { ...pin, id: crypto.randomUUID(), when: pin.when ?? "teď" };
  s.extra.unshift(row);
  writeStore(s);
  return row;
}
...
export function updatePin(id: string, patch: Partial<Pin>): Pin | null {
  const s = readStore();
  const extra = s.extra.find((p) => p.id === id);
  if (extra) {
    Object.assign(extra, patch, { id });
    writeStore(s);
    return extra;
  }
  const seed = SEED.find((p) => p.id === id);
  if (!seed || s.hide.includes(id)) return null;
  const next: Pin = { ...(s.edit[id] ?? seed), ...patch, id };
  s.edit[id] = next;
  writeStore(s);
  return next;
}
...
export function removePin(id: string) {
  const s = readStore();
  if (s.extra.some((p) => p.id === id)) {
    s.extra = s.extra.filter((p) => p.id !== id);
    writeStore(s);
    return;
  }
  if (!s.hide.includes(id)) s.hide.push(id);
  delete s.edit[id];
  writeStore(s);
}
