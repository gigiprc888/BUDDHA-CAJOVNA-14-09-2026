  ...
  return (
    <div className="relative min-h-screen font-sans text-fg">
      <div className="pointer-events-none fixed inset-0 -z-10">
        ...
        <div className="absolute inset-0 opacity-[0.22]" style={{ backgroundImage: "url(/ornament.svg?v=marks)", backgroundSize: "320px 320px" }} />
      </div>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          {list.map((g) => {
            ...
            return (
              <article
                key={g.id}
                className={`rounded-card border bg-surface p-5 transition-[border-color] ${shown ? "border-gold" : "border-gold/20"}`}
              >
                ...
                <p className={"mt-3 text-sm leading-relaxed text-ivory/90 " + (shown ? "" : "line-clamp-2")}>
                  {g.blurb}
                </p>
                {shown ? (
                  <div className="mt-4 space-y-3 border-t border-gold/25 pt-4">
                    <dl className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                      <div>
                        <dt className="text-[10px] tracking-[0.14em] text-muted uppercase">Min. hráčů</dt>
                        <dd className="font-serif text-2xl text-gold">{g.min}</dd>
                      </div>
                      <div>
                        <dt className="text-[10px] tracking-[0.14em] text-muted uppercase">Max. hráčů</dt>
                        <dd className="font-serif text-2xl text-gold">{g.max}</dd>
                      </div>
                      <div>
                        <dt className="text-[10px] tracking-[0.14em] text-muted uppercase">Herní doba</dt>
                        <dd className="font-serif text-2xl text-ivory">{g.time}</dd>
                      </div>
                      <div>
                        <dt className="text-[10px] tracking-[0.14em] text-muted uppercase">Náročnost</dt>
                        <dd className="text-sm text-ivory">
                          {g.difficulty}/5 · {g.difficulty <= 1.5 ? "nenáročná" : g.difficulty < 2.8 ? "střední" : "náročná"}
                        </dd>
                      </div>
                      <div>
                        <dt className="text-[10px] tracking-[0.14em] text-muted uppercase">BoardGameGeek</dt>
                        <dd className="text-sm text-ivory">{g.bgg.toFixed(1)} / 10</dd>
                      </div>
                      <div>
                        <dt className="text-[10px] tracking-[0.14em] text-muted uppercase">Záloha</dt>
                        <dd className="text-sm text-ivory">{GAMES_INTRO.deposit} Kč</dd>
                      </div>
                    </dl>
                    {g.expansion ? (
                      <p className="text-sm text-gold">Hraje se s hlavní hrou z vitríny.</p>
                    ) : null}
                    <p className="text-xs leading-relaxed text-muted">
                      Půjčení z vitríny proti vratné záloze. Obsah si zkontrolujte podle seznamu na vnitřní straně víka.
                      Podložka a lampička na požádání u obsluhy.
                    </p>
                  </div>
                ) : (
