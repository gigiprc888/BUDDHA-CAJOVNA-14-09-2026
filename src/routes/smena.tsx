...
import { LANES, addPin, loadPins, removePin, updatePin, type Lane, type Pin } from "@/lib/board";
import {
  WAVES,
  addWalkIn,
  formatDateCs,
...
function ControlApp({ session, onOut }: { session: ShiftSession; onOut: () => void }) {
  ...
  return (
    <ControlShell page={page} onPage={setPage} who={session.name} role={SHIFT_PINS[session.role].label} onOut={onOut}>
      ...
      {page === "board" ? <BoardAdmin who={session.name} /> : null}
...
function BoardAdmin({ who }: { who: string }) {
  const [lane, setLane] = useState<Lane>("news");
  const [pins, setPins] = useState<Pin[]>(() => (typeof window === "undefined" ? [] : loadPins()));
  const [edit, setEdit] = useState<Pin | null>(null);
  const [draft, setDraft] = useState({ title: "", body: "", tag: "", titleEn: "", bodyEn: "" });
  ...
  function openNew() {
    setEdit({
      ...
      author: who,
      when: "teď",
      tag: lane === "forum" ? "hráč" : lane === "ads" ? "inzerát" : "akce",
    });
    setDraft({ title: "", body: "", tag: lane === "forum" ? "hráč" : lane === "ads" ? "inzerát" : "akce", titleEn: "", bodyEn: "" });
  }

  function openEdit(p: Pin) {
    setEdit(p);
    setDraft({
      title: p.title,
      body: p.body,
  ...
  function save(e: React.FormEvent) {
    e.preventDefault();
    if (!edit || !draft.title.trim() || !draft.body.trim()) return;
    const payload = {
      lane,
      ...
      bodyEn: draft.bodyEn.trim() || undefined,
      when: "teď",
    };
    if (!edit.id) addPin(payload);
    else updatePin(edit.id, payload);
    setEdit(null);
    reload();
  }

  const visible = pins.filter((p) => p.lane === lane);
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-sm text-muted">
        Úpravy vidí hosté na webu v tomto prohlížeči. Majitel i čajovník můžou přidávat, měnit i sundat.
      </p>
      <div className="mt-6 flex flex-wrap gap-2">
        {LANES.map((l) => (
          <button
            ...
            className={`rounded-full px-4 py-1.5 text-[10px] tracking-[0.14em] uppercase ${
              lane === l.id ? "bg-gold text-bg" : "border border-gold/30 text-gold"
            }`}
          >
            {l.label} · {pins.filter((p) => p.lane === l.id).length}
          </button>
        ))}
        <button type="button" onClick={openNew} className="rounded-full bg-gold px-4 py-1.5 text-[10px] tracking-[0.14em] text-bg uppercase">
          + Nový zápis
        </button>
      </div>

      {edit ? (
        <form onSubmit={save} className="mt-5 rounded-card border border-gold bg-surface p-4">
          <div className="flex items-center justify-between">
            <p className="text-[10px] tracking-[0.18em] text-gold uppercase">{edit.id ? "Upravit" : "Nový zápis"} · {who}</p>
            <button type="button" onClick={() => setEdit(null)} className="text-muted hover:text-gold">
              ×
            </button>
          </div>
          <input
            ...
            placeholder="Nadpis"
            className="mt-3 w-full rounded-md border border-gold/25 bg-bg px-3 py-2 text-sm text-ivory outline-none"
          />
          <textarea
            required
            value={draft.body}
            ...
            placeholder="Text"
            rows={4}
            className="mt-2 w-full rounded-md border border-gold/25 bg-bg px-3 py-2 text-sm text-ivory outline-none"
          />
          <div className="mt-2 grid gap-2 md:grid-cols-2">
            ...
            <input
              ...
              placeholder="Nadpis EN (volitelné)"
              className="rounded-md border border-gold/25 bg-bg px-3 py-2 text-sm text-ivory outline-none"
            />
          </div>
          <textarea
      ...
      <ul className="mt-5 space-y-2">
        {visible.length === 0 ? (
          <li className="rounded-card border border-gold/20 p-8 text-center text-sm text-muted">V tomto sloupci nic není.</li>
        ) : (
          visible.map((p) => (
            <li key={p.id} className="rounded-card border border-gold/20 bg-surface p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-[10px] tracking-[0.14em] text-gold uppercase">
                    {p.tag ?? p.lane} · {p.author} · {p.when}
                  </p>
                  <p className="mt-1 font-serif text-xl text-ivory">{p.title}</p>
                  <p className="mt-1 text-sm text-muted">{p.body}</p>
                </div>
                <div className="flex shrink-0 gap-2">
                  <button type="button" onClick={() => openEdit(p)} className="text-[10px] tracking-[0.14em] text-gold uppercase">
                    Upravit
                  </button>
                  <button
                    type="button"
                    ...
                    className="text-[10px] tracking-[0.14em] text-muted uppercase hover:text-red-300"
                  >
                    Sundat
                  </button>
                </div>
